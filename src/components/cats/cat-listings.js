...
const CatListings = (props) => {
  ...
  const paginateCats = (direction) => {
    //increase of decrease offset for fetching
    setOffset(
      direction == "next" ? offset + CATS_PER_PAGE : offset - CATS_PER_PAGE
    );

    //store which page we are on
    setPage(direction == "next" ? page + 1 : page - 1);

    //required for button display functionality
